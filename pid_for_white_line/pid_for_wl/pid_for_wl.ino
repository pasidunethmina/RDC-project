...
float Kp = 0.0575; // Set up the constants value 0.0575
float Ki = 0;
float Kd = 0.1;//0.1
int P;
int I;
...
const uint8_t maxspeeda = 210;//to make fast put 230
const uint8_t maxspeedb = 210;//to make fast put 230
...
void printSensorValues() {
  ...
  for (uint8_t i = 0; i < SensorCount; i++) {
    if (sensorValues[i] > threshold) {
      Serial.print("0 "); // Sensor sees the line
    } else {
      Serial.print("1 "); // Sensor does not see the line
    }
  }
  Serial.println();
}
